...
exports.responseFromCriteriaValidator = [
    ...
    body('nlu.entities', 'An intent is required').optional().isArray(),
    ...
    body('language', 'must be a language code (e.g. "fr" or "en")')
        .optional().isString().isLength({min: 2, max: 2}),
    body('compact', 'must be true/false or 1/0 if set').optional().isBoolean(),
    query('metadata', 'must be 1 or 0 if set').optional().isBoolean(),
]
...
exports.getResponseFromCriteria = async function(req, res) {
    ...
    try {
        ...
        if (req.body.language) {
            ...
            if (!value || !value.sequence.length) throw {
                code: 404, error: 'not_found',
            };
...
exports.allResponsesValidator = [
    query('metadata', 'must be 1 or 0 if set').optional().isBoolean(),
    query('timestamp', 'must be in milliseconds if set').optional().isInt(),
]
